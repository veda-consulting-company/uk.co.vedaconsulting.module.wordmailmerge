...
/*
GK 07032017
 Core file fixed to make postProcess hook work for this Message Template forms
*/
...
class CRM_Admin_Form_MessageTemplates extends CRM_Admin_Form {
  ...
  public function postProcess() {
    ...
    elseif ($this->_action & CRM_Core_Action::VIEW) {
      ...
      // assign Message template id in to $this->_id
      $this->_id = $messageTemplate->id;
      $session = CRM_Core_Session::singleton();

      if ($this->_workflow_id) {
        // Fix to make postProcess hooks work for this form
         $session->pushUserContext(CRM_Utils_System::url('civicrm/admin/messageTemplates', 'selectedChild=workflow&reset=1'));
      }
      else {
        // Fix to make postProcess hooks work for this form
         $session->pushUserContext(CRM_Utils_System::url('civicrm/admin/messageTemplates', 'selectedChild=user&reset=1'));
      }
    }
  }
